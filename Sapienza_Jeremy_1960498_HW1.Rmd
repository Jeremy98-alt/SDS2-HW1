...
points(0:20,dpois(0:20,lambda = post_mean),pch=6,col="blue")
grid()
...
For this setting imagine that I developed an innovative bulb and my goal is to characterize them statistically. We consider 20 innovative bulbs to determine their lifetimes, I observe the follwing data expressed in terms of hours:
...
Based on my experience with light bulbs, I believe that these observations $Y_i$ described their lifetimes using an **exponential distribution** conditionally on $\theta$, the likelihood function is written in this way:
...
For this exercise in order to choose a suitable prior distribution $\pi(\theta)$ we decide to pick the gamma distribution to be able to apply the conjugate analysis, the form is a we can see above.

$$
\pi(\theta) = \frac{r^{s}\theta^{s-1}e^{-r\theta}}{\Gamma(s)} \mathbb{I}_{(0, +\infty)} (\theta) \\
where \,\,\, \mathbb{E}[\theta] = 0.003 \,\,\, and \,\,\, \mathbb{V}[\theta] = (0.00173)^2 
$$

I assume that these choices are only a vague prior opinion because we don't know how these values fits on my $y_1, ..., y_n$. For having a confirmation is necessary to apply the Bayes' rule in order to see how change my uncertainty. The uncertain quantity is the probability distribution that would express one's beliefs about this quantity before some evidence is taken into account.
...
At the end, the prior parameters for the prior distribution are $\pi(\theta) \sim Gamma \Big(r = 1002.37, s = 3.007 \Big)$

As we can seen now we can setup the framework to fit it to the Bayesian analysis and get the posterior parameters, this means:
...
This is a result that we expected due to the behaviour of the prior distribution and the next values obtained in the next chapter. The behaviour of the posterior distribution is shown above.
...
As we can see the actual $\theta$ is probably equal to 0.002.
